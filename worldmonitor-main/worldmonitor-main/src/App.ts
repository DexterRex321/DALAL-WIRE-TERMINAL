  ...
  constructor(containerId: string) {
    ...
    if (storedVariant !== currentVariant) {
      ...
      if (SITE_VARIANT === 'finance') {
        const appliedFinancePreset = localStorage.getItem(FINANCE_PRESET_APPLY_KEY);
        if (appliedFinancePreset !== FINANCE_PRESET) {
          const financeDefaults = new Set(VARIANT_DEFAULTS['finance'] ?? []);
          const rebuiltSettings: Record<string, PanelConfig> = {};
          ...
          for (const key of Object.keys(ALL_PANELS)) {
            ...
            const currentConfig = panelSettings[key];
            rebuiltSettings[key] = {
              ...(currentConfig ?? baseConfig),
              ...baseConfig,
              enabled: isDynamicPanel(key)
                ? currentConfig?.enabled ?? false
                : financeDefaults.has(key),
            };
          }

          panelSettings = rebuiltSettings;
          saveToStorage(STORAGE_KEYS.panels, panelSettings);
          localStorage.setItem(FINANCE_PRESET_APPLY_KEY, FINANCE_PRESET);
          localStorage.setItem('worldmonitor-finance-preset', FINANCE_PRESET);
        }
      }
