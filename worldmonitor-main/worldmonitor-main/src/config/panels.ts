...
import { FINANCE_PRESET, SITE_VARIANT } from './variant';
...
const FINANCE_PANELS: Record<string, PanelConfig> = {
  ...
  'service-status': { name: 'Data & Service Status', enabled: true, priority: 1 },
...
const DALAL_FINANCE_DEFAULTS: string[] = [
  'map',
  'service-status',
  'live-news',
  'insights',
...
export const VARIANT_DEFAULTS: Record<string, string[]> = {
  ...
  finance:   FINANCE_PRESET === 'dalal' ? DALAL_FINANCE_DEFAULTS : Object.keys(FINANCE_PANELS),
...
export const PANEL_CATEGORY_MAP: Record<string, { labelKey: string; panelKeys: string[]; variants?: string[] }> = {
  ...
  finMarkets: {
    labelKey: 'header.panelCatMarkets',
    panelKeys: ['service-status', 'markets', 'markets-news', 'heatmap', 'macro-signals', 'macro-tiles', 'fear-greed', 'fsi', 'yield-curve', 'analysis', 'stock-analysis', 'stock-backtest', 'daily-market-brief', 'polymarket'],
  },
  fixedIncomeFx: {
